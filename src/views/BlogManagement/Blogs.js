...
const Blogs = () => {
  ...
  const getBlogs = async () => {
    try {
      const res = await axios.get('http://18.209.197.35:8000/blogs/get-blogs')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        response = await axios.put(`http://18.209.197.35:8000/blogs/update/${blogId}`, blogSubmitData)
      } else {
        response = await axios.post('http://18.209.197.35:8000/blogs/createBlog', blogSubmitData)
      }
  ...
  const handleDelete = async (blogId) => {
    try {
      const response = await axios.delete(`http://18.209.197.35:8000/blogs/delete/${blogId}`)
