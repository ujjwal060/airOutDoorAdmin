...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://18.209.197.35:3002/api/OrderDetails/');
  ...
  const handleUpdateStatus = async () => {
    try {
      const response = await axios.put(`http://18.209.197.35:3002/api/OrderDetails/${selectedOrder._id}`, { status: newStatus });
