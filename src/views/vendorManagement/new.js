...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://44.196.64.110:3002/api/OrderDetails/');
  ...
  const handleUpdateStatus = async () => {
    try {
      const response = await axios.put(`http://44.196.64.110:3002/api/OrderDetails/${selectedOrder._id}`, { status: newStatus });
