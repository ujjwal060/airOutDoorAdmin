...
const Extension = () => {
  ...
  const fetchExtensions = async () => {
    try {
      const response = await axios.get("http://18.209.197.35:3002/api/Extension/getAll");
  ...
  useEffect(() => {
    if (formVisible) {
      const fetchSpecies = async () => {
        try {
          const response = await axios.get("http://18.209.197.35:3002/api/species/getSpeciesCategories/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://18.209.197.35:3002/api/extension/addExtension", formDataToSend, {
        headers: {
  ...
  const handleEditSubmit = async () => {
    setLoading(true);
    const apiUrl = `http://18.209.197.35:3002/api/Extension/editeExten/${selectedItem._id}`;
                ...
                {extensions.map((extension, index) => (
                  <CTableRow key={extension._id}>
                    ...
                    <CTableDataCell style={{ textAlign: "center" }}>
                      {extension.image && (
                        <img
                          src={`http://18.209.197.35:3002/${extension.image}`} // Update this path according to your API
