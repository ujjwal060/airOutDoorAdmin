...
const AnimalManagement = () => {
  ...
  const fetchAnimals = async (page, search) => {
    setIsLoading(true);
    try {
      const response = await axios.post('http://18.209.197.35:8000/catogries/get', {
        page,
        limit: itemsPerPage,
        search,
      });
  ...
  const addAnimal = async () => {
    ...
    try {
      const response = await axios.post('http://18.209.197.35:8000/catogries/add', formData, {
        headers: {
  ...
  const deleteAnimal = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://18.209.197.35:8000/catogries/delete/${id}`);
      if (response.status === 200) {
